---
id: 04_initiatives__ll_portfolio__03_firm_operations__portfolio_management__readme_md
title: PORTFOLIO_MANAGEMENT — Portfolio Flow Coordination
owner: ML1
status: draft
created_date: 2026-02-26
last_updated: 2026-02-26
tags: []
---

# PORTFOLIO_MANAGEMENT — Portfolio Flow Coordination

## Purpose

Coordinate firm project flow and provide ML1 with advisory portfolio management outputs.

## Scope

### In Scope

- Portfolio flow analysis and sequencing recommendations
- Capacity allocation modeling
- Bottleneck and collision detection

### Out of Scope

- Stage approvals
- Policy decisions
- Operational enforcement

## ML1 Authority Statement

ML1 is the sole authority for project approvals and sequencing decisions.

## Approval State

**READ-ONLY** — Awaiting ML1 population

## Last ML1 Review Date

*Not yet reviewed*
