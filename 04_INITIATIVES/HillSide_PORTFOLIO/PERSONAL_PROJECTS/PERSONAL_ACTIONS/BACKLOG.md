---
id: 04_initiatives__hillside_portfolio__personal_projects__personal_actions__backlog_md
title: Backlog
owner: ML1
status: draft
created_date: 2026-05-03
last_updated: 2026-05-17
tags: [hillside, personal-projects, personal-actions]
---

# Backlog

Items captured but not yet active. Move to ACTIVE_LIST.md when ready to action.

| Item | Context | Linked Project or Area | Date Added | Notes |
|---|---|---|---|---|
| Claim 2025 LL cash flow prize watch | 2025 cash flow target of CAD 160K achieved; ML1 is entitled to a watch prize. Not yet claimed/ordered. Watch: Marathon 41mm CeraShell Quartz Nav-D on Ballistic Nylon Strap. https://www.marathonwatch.com/products/41mm-cerashell-quartz-nav-d-on-ballistic-nylon-strap | HBP-007 | 2026-05-18 | Purchase from Marathon Watch |
| Follow up with financial advisor | ML1 mentioned during session | HBP-001 | 2026-05-03 | — |
| Meeting with Crassula | Product software candidate for Granville (17409052) | HBP-005 | 2026-05-15 | Week of 2026-05-18; exact date TBC |
| Meeting with FinLego | New conversation — payments infrastructure candidate for Granville (17409052) | HBP-005 | 2026-05-15 | Week of 2026-05-18; exact date TBC; FinLego not yet in candidate list |
| Japan home — advance pursuit decision | Active project in `JAPAN_HOME_DECISION` + 7 properties logged in `JAPAN_PROPERTY_PIPELINE`. Decision gate not yet passed. Next: ML1 to review pipeline and make pursue / defer / no-go call. | JAPAN_HOME_DECISION | 2026-05-17 | — |
| Recruit Dominion Partners delivery person | Need one person who can deliver the entire core offer (Canada market entry; Canadian trade diversification away from US). Current network candidates not able to deliver. Target profile: Indian MBA with cross-border trade / market-entry experience; Consultant to Senior Consultant level. | 175 / Dominion Partners | 2026-05-17 | — |

