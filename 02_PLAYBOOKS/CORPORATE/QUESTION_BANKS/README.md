---
id: 02_playbooks__corporate__question_banks__readme_md
title: Corporate Question Banks
owner: ML1
status: draft
created_date: 2026-02-08
last_updated: 2026-02-08
tags: []
---

## Playbook Header
Playbook ID: 02_playbooks__corporate__question_banks__readme_md
Version: 1.0
Status: draft

Principles Referenced: PRN-001, PRN-002, PRN-003, PRN-004, PRN-006, PRN-009
Policies Applied: POL-004, POL-006, POL-009, POL-011
Protocols Enforced: PRO-004, PRO-006, PRO-009, PRO-011
Doctrine Invoked: 00_SYSTEM/constitution.md, 01_DOCTRINE/index.yaml

Inputs: TBD
Outputs: TBD
Acceptance Criteria: TBD


# Corporate Question Banks

Structured questions for intake, diligence, and risk assessment.

---

## Categories

- **Intake** — Initial client meeting questions
- **Diligence** — Document and information requests
- **Risk** — Red flag identification

---

## Question Bank Format

```markdown
# [QUESTION BANK NAME]

## Context
When to use this bank

## Questions

### Section 1
1. Question text?
   - Follow-up if yes: ...
   - Follow-up if no: ...
   - Red flag: ...

### Section 2
1. Question text?
```

---

## Index

| Bank | Use Case |
|------|----------|
| TBD | |
